...
<section class="px-6 py-10 sm:py-16">
    <div class="mx-auto max-w-4xl overflow-hidden rounded-2xl border border-[#e7edf3] dark:border-white/10 shadow-sm">
        ...
        <div class="grid gap-3 p-4 sm:grid-cols-2 sm:gap-4 sm:p-6">
            ...
            @endforeach
            <div class="flex items-center justify-between gap-3 rounded-xl border border-brand-blue-tint bg-brand-blue-tint dark:bg-white/10 px-3 py-2.5 text-brand-blue sm:px-4 sm:py-3">
            ...
            @foreach([
                ...
                <div class="flex items-start gap-3 rounded-xl bg-brand-red-tint dark:bg-brand-red/15 px-3 py-2.5 sm:px-4 sm:py-3">
                    ...
                    <span>
                        ...
                        <span class="text-xs text-[#45627d] dark:text-white/60 sm:text-sm">{{ $body }}</span>
                    </span>
                </div>
            @endforeach
        </div>
    ...
    <div class="mx-auto mt-6 max-w-4xl rounded-2xl border border-[#e7edf3] dark:border-white/10 p-4 shadow-sm sm:mt-8 sm:p-8">
        ...
        <div class="mt-4 grid gap-2.5 sm:mt-5 sm:gap-3">
            @forelse($hospitals as $hospital)
                <div class="rounded-xl bg-brand-red-tint dark:bg-brand-red/15 px-3 py-2.5 sm:px-4 sm:py-3">
                    <div class="flex items-center justify-between gap-3">
                        <span class="text-sm font-bold text-[#062238] dark:text-[#e0e0e0] sm:text-base">{{ $hospital['name'] }}</span>
                    ...
                    @if(!empty($hospital['address']))
                        <p class="mt-1 text-xs text-[#45627d] dark:text-white/60 sm:text-sm">{{ $hospital['address'] }}</p>
                    @endif
            ...
            @empty
                <p class="text-sm text-[#45627d] dark:text-white/60">Hospital details haven't been added yet &mdash; add your nearest hospitals from Admin &rarr; Sections.</p>
            @endforelse
        </div>
    </div>
</section>
