...
@section('og_image', $service->coverImage() ? image_url($service->coverImage()) : asset('images/hero-team.jpg'))
...
<section class="px-6 py-24">
    <div class="mx-auto max-w-4xl" data-reveal>
        ...
        <h1 style="{{ text_style($service->text_styles, 'title') }}" class="mt-7 font-serif text-4xl font-bold text-ink dark:text-[#e0e0e0] md:text-5xl">{{ $service->title }}</h1>
        <p style="{{ text_style($service->text_styles, 'short_description') }}" class="mt-5 text-lg leading-8 text-ink-muted dark:text-white/60">{{ $service->short_description }}</p>
        @if($service->description)
            <div style="{{ text_style($service->text_styles, 'description') }}" class="prose mt-8 leading-8 text-ink-muted dark:text-white/60">{!! nl2br(e($service->description)) !!}</div>
        @endif
    ...
    @if($others->isNotEmpty())
        <div class="mx-auto mt-20 max-w-6xl">
            <h2 class="font-serif text-2xl font-bold text-ink dark:text-[#e0e0e0]">Other services</h2>
