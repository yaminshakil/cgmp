...
<section class="bg-white px-6 py-16 text-ink dark:bg-[#0b0b0b] dark:text-white md:py-24">
    <div class="mx-auto grid max-w-[1200px] gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] lg:gap-20">
        <div data-reveal="left" class="lg:sticky lg:top-28 lg:self-start">
            ...
            <p class="mt-5 max-w-md text-sm leading-6 text-ink-faint dark:text-white/55 sm:text-base sm:leading-7">Clear answers about our services, booking and policies, so you know what to expect before you visit {{ setting('clinic_name') }}.</p>
            <div class="mt-8 flex flex-wrap items-center gap-3">
                @if($showAllLink)
                    <a href="{{ route('faq') }}" class="btn-lift inline-flex items-center gap-2 rounded-xl border border-ink/25 px-6 py-3 text-sm font-semibold text-ink hover:bg-ink/5 dark:border-white/25 dark:text-white dark:hover:bg-white/10">
                        View all FAQs
        ...
        <div class="grid content-start gap-4">
            ...
            @empty
                <p class="text-ink-faint dark:text-white/60">No FAQs yet.</p>
            @endforelse
        </div>
    </div>
</section>
