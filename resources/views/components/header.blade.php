    ...
    <header id="site-header" class="relative z-10 border-b border-gray-100 bg-white/95 backdrop-blur transition-colors duration-300 dark:border-white/10 dark:bg-[#1a1a1a]/95" :class="scrolled ? 'shadow-[0_6px_24px_rgba(15,42,67,0.16)] dark:shadow-[0_6px_24px_rgba(255,255,255,0.10)]' : 'shadow-[0_2px_16px_rgba(15,42,67,0.08)] dark:shadow-[0_2px_16px_rgba(255,255,255,0.07)]'">
        <div class="flex items-center justify-between gap-4 pl-4 pr-6 site-header__inner transition-[padding] duration-300" :class="scrolled ? 'py-2' : 'py-3'">
            ...
            <div class="ml-auto flex items-center gap-3 lg:ml-0">
                <a href="tel:{{ preg_replace('/\s+/', '', setting('phone', '')) }}" class="hidden items-center gap-2 text-sm font-semibold text-[#3b4a5a] transition-colors hover:text-brand-blue dark:text-white/80 dark:hover:text-white lg:flex">
                ...
                <div class="hidden h-6 w-px bg-gray-200 dark:bg-white/10 lg:block"></div>
                ...
                <button aria-label="Open menu" class="relative h-9 w-9 shrink-0 rounded-full transition-colors hover:bg-gray-100 dark:hover:bg-white/10 lg:hidden" @click="open = !open">
                    ...
                    <span class="absolute left-1/2 top-1/2 block h-0.5 w-5 -translate-x-1/2 rounded-full bg-[#062238] transition-all duration-200 dark:bg-white" :class="open ? 'opacity-0' : 'opacity-100'"></span>
                    <span class="absolute left-1/2 top-1/2 block h-0.5 w-5 -translate-x-1/2 rounded-full bg-[#062238] transition-all duration-300 dark:bg-white" :class="open ? 'translate-y-0 -rotate-45' : 'translate-y-[6px]'"></span>
                </button>
            </div>
        </div>
        ...
        >
            ...
            <button
                type="button"
                @click="toggleTheme()"
                class="flex items-center gap-3 rounded-lg px-3 py-3 font-medium tracking-wide text-[#3b4a5a] transition-colors duration-200 hover:bg-brand-blue-tint dark:text-white/70 dark:hover:bg-white/5"
            >
                <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-blue-tint text-brand-blue dark:bg-white/10 dark:text-[#e0e0e0]">
