...
<div {{ $attributes->merge(['class' => 'flex flex-col items-center gap-1 ' . ($light ? 'text-white' : 'text-brand-blue')]) }}>
    @if(setting('logo_path'))
        <img src="{{ image_url(setting('logo_path')) }}" alt="{{ setting('clinic_name', 'Clinic logo') }}" class="h-[66px] w-[120px] shrink-0 object-contain">
    @else
        <div class="relative h-[66px] w-[120px] shrink-0">
