...
<a href="{{ booking_url() }}" data-book-appointment @if(booking_is_external() && ! setting('healthengine_id')) target="_blank" rel="noopener" @endif {{ $attributes->merge(['class' => "bg-brand-red text-white hover:bg-brand-red-dark btn-lift inline-flex items-center gap-3 rounded-2xl {$padding} font-bold shadow-lg"]) }}>
