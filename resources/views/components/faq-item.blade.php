...
<details @if($open) open @endif class="group rounded-xl bg-[#f4f6f8] transition-colors duration-300 open:bg-[#eaf0f6] dark:bg-[#1c1c1c] dark:open:bg-[#242424]" data-reveal>
    <summary class="flex cursor-pointer list-none items-center justify-between gap-6 px-5 py-5 text-[#062238] dark:text-white sm:px-7 sm:py-6 [&::-webkit-details-marker]:hidden">
        <span style="{{ text_style($styles, 'question') }}" class="min-w-0 text-base font-medium leading-snug sm:text-lg">{{ $question }}</span>
        <span class="relative h-5 w-5 shrink-0 text-[#062238] dark:text-white transition-transform duration-300 group-open:rotate-180" aria-hidden="true">
    ...
    <p style="{{ text_style($styles, 'answer') }}" class="px-5 pb-6 text-sm leading-6 text-[#45627d] dark:text-white/65 sm:px-7 sm:pb-7 sm:text-[15px] sm:leading-7">{{ $answer }}</p>
</details>
